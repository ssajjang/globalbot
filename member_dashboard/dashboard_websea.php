...
define('WEBSEA_API_URL', 'https://oapi.websea.com');

$conn = get_db_connection();

// [핵심 교정] 현재 실행 중인 파일명을 동적으로 가져옵니다. (하드코딩 의존성 제거)
...
CREATE TABLE IF NOT EXISTS rx_daily_record (
    ...
    uid VARCHAR(50) NOT NULL COMMENT '회원 UID',
    record_date DATE NOT NULL COMMENT '기록 일자 (YYYY-MM-DD)',
    week_tag VARCHAR(10) NOT NULL DEFAULT '' COMMENT '주간분류(YYYY-Www)',
    daily_pnl DECIMAL(20,6) NOT NULL DEFAULT '0.000000' COMMENT '당일 순수익',
    investment_amount DECIMAL(20,4) NOT NULL DEFAULT '0.0000' COMMENT '당시 기준 투자금',
    roi DECIMAL(10,4) NOT NULL DEFAULT '0.0000' COMMENT '당일 수익률(%)',
    updated_at DATETIME NOT NULL COMMENT '최근 갱신 시간',
    PRIMARY KEY (idx),
    UNIQUE KEY uniq_uid_date (uid, record_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
";
$conn->query($sql_create_daily);

$sql_create_history = "
CREATE TABLE IF NOT EXISTS rx_account_history (
    idx INT(11) NOT NULL AUTO_INCREMENT,
    rx_uid VARCHAR(100) NOT NULL,
    rx_ce TINYINT(1) NOT NULL DEFAULT 3,
    amount_type ENUM('futures', 'spot') DEFAULT 'futures' COMMENT '금액 타입 (futures: 투자금, spot: 유보금)',
    before_amount FLOAT(11,4) NOT NULL DEFAULT 0,
    after_amount FLOAT(11,4) NOT NULL DEFAULT 0,
    regdate DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (idx),
    KEY idx_uid_ce (rx_uid, rx_ce)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
";
$conn->query($sql_create_history);

// 주간 TXID 저장 테이블 생성
$sql_create_weekly_txid = "
CREATE TABLE IF NOT EXISTS rx_weekly_txid (
    idx INT(11) NOT NULL AUTO_INCREMENT,
    uid VARCHAR(50) NOT NULL COMMENT '회원 UID',
    week_tag VARCHAR(20) NOT NULL COMMENT '주간분류(YYYY-Www)',
    txid VARCHAR(255) NOT NULL COMMENT 'TXID 값',
    regdate DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (idx),
    UNIQUE KEY uniq_uid_week (uid, week_tag)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
";
$conn->query($sql_create_weekly_txid);

// [핵심 교정] DB 컬럼 안전 생성: DB 저장 실패 및 컬럼 누락 방지
function add_column_if_not_exists($db_conn, $table, $column, $definition) {
    $check = $db_conn->query("SHOW COLUMNS FROM `{$table}` LIKE '{$column}'");
    if ($check && $check->num_rows == 0) {
        $db_conn->query("ALTER TABLE `{$table}` ADD COLUMN `{$column}` {$definition}");
    }
}

add_column_if_not_exists($conn, 'rx_account_history', 'amount_type', "ENUM('futures', 'spot') DEFAULT 'futures' COMMENT '금액 타입' AFTER rx_ce");
add_column_if_not_exists($conn, 'rx_member', 'rx_spot_acount', "DECIMAL(20,4) NOT NULL DEFAULT '0.0000' COMMENT '현물 유보금' AFTER rx_acount");
add_column_if_not_exists($conn, 'rx_member', 'rx_reserve_percent', "TINYINT(3) NOT NULL DEFAULT 0 COMMENT '유보율(0,30,100)' AFTER rx_spot_acount");
*/
...
if (isset($_GET['admin_view']) && $_GET['admin_view'] == '1' && isset($_SESSION['is_admin']) && $_SESSION['is_admin'] === true) {
    ...
    $admin_ce = intval($_GET['admin_ce'] ?? 3);

    if (!empty($admin_uid)) {
        $uid_safe = $conn->real_escape_string($admin_uid);
        $res_tmp = $conn->query("SELECT * FROM rx_member WHERE rx_uid = '{$uid_safe}' AND rx_ce = {$admin_ce} LIMIT 1");
...
if (!$admin_view && intval($member['rx_ce']) !== 3) {
...
$s_uid = $member['rx_uid'];
$current_ce = $admin_view ? ($admin_ce ?? 3) : intval($member['rx_ce'] ?? 3);
if (isset($_GET['ajax_action'])) {
    $current_ce = isset($_GET['admin_ce']) ? intval($_GET['admin_ce']) : 3;
}

$current_investment_amount = floatval($member['rx_acount'] ?? 0); // 선물 투자금
$current_spot_amount = floatval($member['rx_spot_acount'] ?? 0); // 현물 유보금
$s_symbol = isset($_REQUEST['s_symbol']) ? trim($_REQUEST['s_symbol']) : "BTC-USDT";

$api_key = preg_replace('/\s+/', '', $member['rx_apikey'] ?? '');
$ss_key  = preg_replace('/\s+/', '', $member['rx_sskey'] ?? '');
$exchange_name = defined('EXCHANGE_MAP') ? (EXCHANGE_MAP[$current_ce] ?? 'WebSea') : 'WebSea';
$keys = ['rx_apikey' => $api_key, 'rx_sskey' => $ss_key, 'mb_id' => $s_uid];
...
global $rx_debug_logs;
$rx_debug_logs = [];

function rx_add_debug($title, $data = null) {
    global $rx_debug_logs;
    $rx_debug_logs[] = ['time' => date('H:i:s'), 'title' => $title, 'data' => $data];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act == 'update_settings') {
        ...
        $chk_res = $conn->query("SELECT rx_acount, rx_spot_acount FROM rx_member WHERE rx_uid = '{$uid_safe}' AND rx_ce = {$current_ce} LIMIT 1");
        ...
        if (abs($old_acount - $up_acount) >= 0.0001) {
            $conn->query("INSERT INTO rx_account_history (rx_uid, rx_ce, amount_type, before_amount, after_amount, regdate) VALUES ('{$uid_safe}', {$current_ce}, 'futures', {$old_acount}, {$up_acount}, NOW())");
        }

        if (abs($old_spot - $up_spot_acount) >= 0.0001) {
            $conn->query("INSERT INTO rx_account_history (rx_uid, rx_ce, amount_type, before_amount, after_amount, regdate) VALUES ('{$uid_safe}', {$current_ce}, 'spot', {$old_spot}, {$up_spot_acount}, NOW())");
        }

        // 유보율 필드 업데이트 포함
        $up_sql = "UPDATE rx_member SET rx_apikey = '{$up_api}', rx_sskey = '{$up_sec}', rx_acount = {$up_acount}, rx_spot_acount = {$up_spot_acount}, rx_reserve_percent = {$up_reserve_percent} WHERE rx_uid = '{$uid_safe}' AND rx_ce = {$current_ce}";

        $add_params = $admin_view ? "&admin_view=1&admin_uid=".urlencode($s_uid)."&admin_ce={$current_ce}" : "";

        if ($conn->query($up_sql)) {
            // [핵심 교정] 로그인 할 때와 동일한 로직: DB 업데이트 후 세션 정보 강제 최신화
            ...
            $db_error = $conn->error;
            $redirect_url = "{$current_page}?s_symbol=".urlencode($s_symbol)."{$add_params}&msg=db_error_" . urlencode($db_error) . "#tab-settings";
        }
		...
		if ($member1) {
			...
			if ($sync_status !== 200) {
				error_log("[websea sync] 147번 동기화 실패 HTTP:{$sync_status} UID:{$member1['rx_uid']}");
			}
		}
    ...
    if ($act == 'save_txid') {
        ...
        if (!empty($week_tag) && !empty($txid)) {
            $conn->query("INSERT INTO rx_weekly_txid (rx_uid, week_tag, txid, regdate) VALUES ('{$uid_safe}', '{$week_tag}', '{$txid}', NOW()) ON DUPLICATE KEY UPDATE txid = '{$txid}', regdate = NOW()");
			//echo "INSERT INTO rx_weekly_txid (rx_uid, week_tag, txid, regdate) VALUES ('{$uid_safe}', '{$week_tag}', '{$txid}', NOW()) ON DUPLICATE KEY UPDATE txid = '{$txid}', regdate = NOW()";
        }
    ...
    // 3. 자산 이동
    if ($act == 'transfer') {
        $res_trans = websea_transfer_asset($keys, trim($_POST['t_type']), trim($_POST['t_amount']));

        if ($res_trans['status']) {
            $msg_code = "trans_ok_" . urlencode(trim($_POST['t_amount']));
        } else {
    ...
    // 4. 출금
    if ($act == 'withdraw') {
        $res_withdraw = websea_withdraw_asset($keys, trim($_POST['w_currency']), trim($_POST['w_chain']), trim($_POST['w_address']), trim($_POST['w_amount']), trim($_POST['w_memo'] ?? ''));

        if ($res_withdraw['status']) {
            $withdraw_id = $res_withdraw['data']['withdrawId'] ?? t('lbl_completed');
            $msg_code = "withdraw_ok_" . urlencode($withdraw_id);
        } else {
            $msg_code = "withdraw_fail_" . urlencode($res_withdraw['msg']);
        }
...
// [WebSea API 함수들]
// ==================================================================
function websea_generate_nonce() {
    $chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
    $randStr = '';
    for ($i = 0; $i < 5; $i++) { $randStr .= $chars[mt_rand(0, strlen($chars) - 1)]; }
    return time() . '_' . $randStr;
}

function websea_generate_signature($nonce, $token, $secret, $params) {
    $tmpArr = array($token, $secret, $nonce);
    if (!empty($params) && is_array($params)) {
        foreach ($params as $k => $v) { $tmpArr[] = $k . "=" . $v; }
    }
    sort($tmpArr, SORT_STRING);
    return sha1(implode('', $tmpArr));
}

function websea_api_call($method, $endpoint, $params = [], $keys = []) {
    $url = WEBSEA_API_URL . $endpoint;
    $token = $keys['rx_apikey'] ?? '';
    $secret = $keys['rx_sskey'] ?? '';
    $nonce = websea_generate_nonce();
    $signature = websea_generate_signature($nonce, $token, $secret, $params);

    $headers = [ 'Token: ' . $token, 'Nonce: ' . $nonce, 'Signature: ' . $signature ];
    $ch = curl_init();
    ...
    if ($curl_error) return ['status' => false, 'msg' => t('err_comm') . ": {$curl_error}", 'http_code' => 0];
    if ($response === "" || strtoupper($response) === "OK") return ['status' => true, 'data' => []];

    $result = json_decode($response, true);
    if ($result === null) return ['status' => false, 'msg' => t('err_json_parse') . ' Raw: ' . $response];

    $is_success = false;
    $errno_str = isset($result['errno']) ? (string)$result['errno'] : '';
    if ($errno_str === '0' || $errno_str === '20538' || (isset($result['code']) && (string)$result['code'] === '200')) $is_success = true;

    $check_msg = strtolower($result['errmsg'] ?? ($result['msg'] ?? ''));
    if (strpos($check_msg, 'success') !== false || strpos($check_msg, 'succeed') !== false || $check_msg === 'ok') $is_success = true;

    if ($is_success) {
        $data = $result['result'] ?? ($result['data'] ?? []);
        if (isset($keys['mb_id'])) {
            if (isset($data[0]) && is_array($data[0])) $data[0]['mb_id'] = $keys['mb_id'];
            if (isset($data[1]) && is_array($data[1])) $data[1]['mb_id'] = $keys['mb_id'];
        }
        return ['status' => true, 'data' => $data, 'http_code' => $http_code];
    } else {
        return ['status' => false, 'msg' => t('lbl_err_occurred') . ": " . ($result['errmsg'] ?? ($result['msg'] ?? 'Unknown Error'))];
    }
}

function websea_transfer_asset($keys, $direction, $amount, $asset = 'USDT') {
    if ($amount <= 0) return ['status' => false, 'msg' => t('err_amt_gt_zero')];
    return websea_api_call('POST', '/v1/wallet/transfer', ['transType' => ($direction == 1) ? 'SPOT_FUTURE' : 'FUTURE_SPOT', 'amount' => $amount, 'asset' => $asset], $keys);
}

function websea_withdraw_asset($keys, $currency, $chain, $address, $amount, $memo = '') {
    $params = ['currency' => strtoupper($currency), 'chain' => strtoupper($chain), 'address' => $address, 'amount' => $amount];
    if (!empty($memo)) $params['memo'] = $memo;
    return websea_api_call('POST', '/v1/wallet/withdraw', $params, $keys);
}

function websea_get_spot_balance($keys) {
    $res = websea_api_call('GET', '/v1/wallet/list', ['currency' => 'USDT', 'show_all' => 1], $keys);
    if ($res['status'] && !empty($res['data'])) {
        $mapped_free = 0; $mapped_locked = 0;
        foreach ($res['data'] as $v) {
            if (isset($v['currency']) && strtoupper($v['currency']) === 'USDT') {
                $mapped_free = floatval($v['available'] ?? 0);
                $mapped_locked = floatval($v['frozen'] ?? 0);
                break;
            }
        }
        $res['data'] = ['free' => $mapped_free, 'locked' => $mapped_locked];
    } else {
        $res['data'] = ['free' => 0, 'locked' => 0];
    }
    return $res;
}

function websea_get_futures_balance($keys) {
    $res = websea_api_call('GET', '/v1/futures/wallet', [], $keys);
    if ($res['status'] && !empty($res['data'])) {
        $mapped_balance = 0; $mapped_avail = 0;
        foreach ($res['data'] as $v) {
            if (isset($v['asset']) && strtoupper($v['asset']) === 'USDT') {
                $mapped_balance = floatval($v['balance'] ?? 0) + floatval($v['unPnl'] ?? 0);
                $mapped_avail = floatval($v['avail'] ?? 0);
                break;
            }
        }
        if ($mapped_balance == 0 && isset($res['data'][0])) {
            $mapped_balance = floatval($res['data'][0]['balance'] ?? 0) + floatval($res['data'][0]['unPnl'] ?? 0);
            $mapped_avail = floatval($res['data'][0]['avail'] ?? 0);
        }
        $res['data'] = ['balance' => $mapped_balance, 'availableBalance' => $mapped_avail];
    } else {
        $res['data'] = ['balance' => 0, 'availableBalance' => 0];
    }
    return $res;
}

function websea_get_current_positions($keys, $symbol = '') {
    $params = [];
    if ($symbol) $params['symbol'] = $symbol;

    $res = websea_api_call('GET', '/v1/futures/position', $params, $keys);
    $ticker_res = websea_api_call('GET', '/v1/futures/24hr', $params, $keys);

    $current_prices = [];
    if ($ticker_res['status'] === true && !empty($ticker_res['data'])) {
        foreach ($ticker_res['data'] as $t) {
            if (isset($t['symbol'])) $current_prices[$t['symbol']] = floatval($t['last_price'] ?? 0);
        }
    }
    ...
    if ($res['status'] === true && !empty($res['data'])) {
        ...
        foreach ($res['data'] as $v) {
            ...
            $side_str = '-';
            if (isset($v['type'])) {
                if ($v['type'] == 1) $side_str = 'LONG';
                elseif ($v['type'] == 2) $side_str = 'SHORT';
            }

            $lever = isset($v['lever_rate']) ? intval($v['lever_rate']) : (isset($v['leverage']) ? intval($v['leverage']) : (isset($v['lever']) ? intval($v['lever']) : 50));

            $multiplier = 1;
            if (strpos(strtoupper($sym), 'BTC') !== false) $multiplier = 0.001;
            elseif (strpos(strtoupper($sym), 'ETH') !== false) $multiplier = 0.01;
            elseif (strpos(strtoupper($sym), 'XRP') !== false) $multiplier = 10;

            $real_qty = (isset($v['amount']) ? floatval($v['amount']) : 0) * $multiplier;
            $avgPrice = isset($v['open_price_avg']) ? floatval($v['open_price_avg']) : 0;

            $unrealizedPnL = floatval($v['un_profit'] ?? 0);
            $bood = floatval($v['bood'] ?? ($v['margin'] ?? 0));
            $profitRate = isset($v['profit_rate']) ? floatval($v['profit_rate']) : ($bood > 0 ? ($unrealizedPnL / $bood) : 0);

            $markPrice = $current_prices[$sym] ?? 0;

            $mapped_data[] = [
                'symbol'        => $sym,
                'mb_id'         => $v['mb_id'] ?? ($keys['mb_id'] ?? ''),
                'side'          => $side_str,
                'leverage'      => $lever,
                'position'      => $v['amount'] ?? 0,
                'avgPrice'      => $avgPrice,
                'flp'           => $v['liquidation_price'] ?? 0,
                'unrealizedPnL' => $unrealizedPnL,
                'profitRate'    => $profitRate,
                'markPrice'     => $markPrice
            ];
        }
        $res['data'] = $mapped_data;
    }
    return $res;
}

function websea_get_history_positions($keys, $symbol = 'BTC-USDT') {
    $all_history = [];
    $page_num = 1;
    $max_pages = 10;
    ...
    for ($i = 0; $i < $max_pages; $i++) {
        ...
        $res = websea_api_call('GET', '/v1/futures/orders/history', $params, $keys);

        if (!$res['status'] || empty($res['data'])) break;

        $batch_mapped = [];
        foreach ($res['data'] as $v) {
            if (isset($v['contract_type']) && $v['contract_type'] === 'close') {
                $side = '-';
                if (isset($v['type'])) {
                    if (strpos($v['type'], 'buy') !== false) $side = 'SHORT';
                    elseif (strpos($v['type'], 'sell') !== false) $side = 'LONG';
                }

                $raw_pnl = floatval($v['profit'] ?? ($v['realized_profit'] ?? 0));
                $lever = isset($v['lever_rate']) ? intval($v['lever_rate']) : (isset($v['leverage']) ? intval($v['leverage']) : (isset($v['lever']) ? intval($v['lever']) : 50));
                $close_avg_price = floatval($v['price_avg'] ?? ($v['price'] ?? 0));

                $deal_amt = floatval($v['deal_amount'] ?? ($v['amount'] ?? 0));
                $symbol_upper = strtoupper($v['symbol'] ?? '');
                $multiplier = 1;
                ...
                $size_in_coins = $deal_amt * $multiplier;

                // 역산 기반 진입가(Open Price) 도출
                $open_avg_price = 0;
                if ($size_in_coins > 0) {
                    if ($side === 'LONG') {
                        $open_avg_price = $close_avg_price - ($raw_pnl / $size_in_coins);
                    } elseif ($side === 'SHORT') {
                        $open_avg_price = $close_avg_price + ($raw_pnl / $size_in_coins);
                    }
                }

                // 수수료 차감 적용 (Net Profit 도출)
                $open_value = $open_avg_price * $size_in_coins;
                $close_value = $close_avg_price * $size_in_coins;

                $api_fee = floatval($v['fee'] ?? 0);
                $total_fee = ($api_fee > 0) ? $api_fee : (($open_value + $close_value) * 0.0006);

                $net_profit = $raw_pnl - $total_fee;

                // Net ROI 산출
                $net_roi = 0;
                if ($lever > 0 && $open_value > 0) {
                    $margin = $open_value / $lever;
                    $net_roi = $net_profit / $margin;
                }

                $batch_mapped[] = [
                    'order_id'       => $v['order_id'] ?? '',
                    'closeTime'      => (isset($v['ctime']) ? (floatval($v['ctime']) > 1e11 ? $v['ctime'] : floatval($v['ctime']) * 1000) : 0),
                    'symbol'         => $v['symbol'] ?? '',
                    'side'           => $side,
                    'leverage'       => $lever,
                    'openAvgPrice'   => $open_avg_price,
                    'closeAvgPrice'  => $close_avg_price,
                    'realizedPnL'    => $net_profit,
                    'realizedPnlRate'=> $net_roi
                ];
            }
        }

        $all_history = array_merge($all_history, $batch_mapped);

        if (!empty($batch_mapped)) {
            $last_item = end($batch_mapped);
            if (isset($last_item['closeTime']) && $last_item['closeTime'] > 0) {
                if (($last_item['closeTime'] / 1000) < $one_month_ago) break;
            }
        }
    ...
    foreach ($all_history as $item) {
        $key = $item['order_id'] ?? serialize($item);
        $unique[$key] = $item;
    }

    return ['status' => true, 'data' => array_values($unique)];
}

function rx_get_bot_initial_qty_websea($keys, $symbol) {
    if (empty($keys['rx_apikey'])) return 0;
    $res = websea_api_call('GET', '/v1/futures/orders/history', ['symbol' => $symbol, 'limit' => 10], $keys);

    $min_qty = null;
    if ($res['status'] === true && !empty($res['data'])) {
        foreach ($res['data'] as $v) {
            $qty = floatval($v['amount'] ?? ($v['deal_amount'] ?? 0));
            if ($qty > 0) {
                if ($min_qty === null || $qty < $min_qty) $min_qty = $qty;
            }
        }
    }
...
if (isset($_GET['ajax_action']) && $_GET['ajax_action'] === 'get_positions') {
    ...
    $ajax_symbol = trim($_GET['symbol'] ?? '');

    if (!empty($api_key) && !empty($ss_key)) {
        $result = websea_get_current_positions($keys, $ajax_symbol);
        if ($result['status'] === true && !empty($result['data'])) {
            foreach ($result['data'] as $key => $row) {
                $result['data'][$key]['initial_qty'] = rx_get_bot_initial_qty_websea($keys, $row['symbol']);
            }
        }
...
// ==================================================================
// [AJAX] 실제 자산 조회 (모달 검증용) 엔드포인트 추가 (WebSea)
// ==================================================================
...
if (!$fatal_error) {
    ...
    if (!$fatal_error) {
        ...
        if ($res_hist['status']) {
            ...
            foreach ($api_daily_pnl as $d_date => $d_pnl) {
                ...
                $chk_res = $conn->query("SELECT investment_amount FROM rx_daily_record WHERE uid = '{$uid_safe}' AND record_date = '{$d_date}'");

                if ($chk_res && $chk_res->num_rows > 0) {
                    $existing_equity = floatval($chk_res->fetch_assoc()['investment_amount']);
                    ...
                    $conn->query("UPDATE rx_daily_record SET daily_pnl = {$d_pnl}, investment_amount = {$target_equity}, roi = {$target_roi}, updated_at = NOW() WHERE uid = '{$uid_safe}' AND record_date = '{$d_date}'");
                } else {
                    $target_roi = ($current_investment_amount > 0) ? ($d_pnl / $current_investment_amount) * 100 : 0;
                    $conn->query("INSERT INTO rx_daily_record (uid, record_date, week_tag, daily_pnl, investment_amount, roi, updated_at) VALUES ('{$uid_safe}', '{$d_date}', '{$week_tag}', {$d_pnl}, {$current_investment_amount}, {$target_roi}, NOW())");
                }
            }
    ...
    $db_daily_pnl = []; $db_daily_equity = []; $db_daily_roi = [];

    if (!$fatal_error) {
        $uid_safe = $conn->real_escape_string($s_uid);
        $res_load = $conn->query("SELECT record_date, investment_amount, daily_pnl, roi FROM rx_daily_record WHERE uid = '{$uid_safe}' ORDER BY record_date ASC");
        ...
        if ($res_load && $res_load->num_rows > 0) {
            ...
            while ($row = $res_load->fetch_assoc()) {
                ...
                if ($d == $today_str) { $pnl_today = $row['daily_pnl']; $roi_today_pct = $row['roi']; }
                if ($d == $yesterday_str) { $pnl_yesterday = $row['daily_pnl']; $roi_yesterday_pct = $row['roi']; }
                if ($d >= $date_30_ago) { $pnl_30days += $row['daily_pnl']; }
            ...
            $member_regdate = $member['mm_regdate'] ?? null;
            if ($member_regdate) {
                $reg_date_str = date('Y-m-d', strtotime($member_regdate));
                $total_trading_days = floor((strtotime($today_str) - strtotime($reg_date_str)) / 86400) + 1;
                if ($total_trading_days < 1) $total_trading_days = 1;
                if ($first_trade_date === null || $reg_date_str < $first_trade_date) $first_trade_date = $reg_date_str;
            } elseif ($first_trade_date) {
                $total_trading_days = floor((strtotime($today_str) - strtotime($first_trade_date)) / 86400) + 1;
            }
...
// 주간 TXID 매핑 데이터 로드 (주간 수익 탭용)
$txid_map = [];
if (!$fatal_error) {
    $res_tx = $conn->query("SELECT week_tag, txid FROM rx_weekly_txid WHERE uid = '".$conn->real_escape_string($s_uid)."'");
...
function renderSnapshotCalendar($year, $month, $db_pnl, $db_roi) {
    ...
    foreach ([t('cal_sun'), t('cal_mon'), t('cal_tue'), t('cal_wed'), t('cal_thu'), t('cal_fri'), t('cal_sat')] as $idx => $day) {
    ...
    for ($day = 1; $day <= $daysInMonth; $day++) {
        ...
        $dayClass = 'calendar-day' . ($dateStr == $today ? ' today' : '') . ($dayOfWeek == 0 ? ' sunday' : '') . ($dayOfWeek == 6 ? ' saturday' : '');
        ...
        $html .= "<div class='{$dayClass}'><div class='day-number'>{$day}</div><div class='day-pnl {$pnlClass}'>{$pnlDisplay}</div>" . ($roiDisplay ? "<div class='day-roi {$pnlClass}'>{$roiDisplay}</div>" : "") . "</div>";
    }
...
function renderWeeklyTable($db_pnl, $db_equity, $txid_map) {
    ...
    foreach ($db_pnl as $date => $pnl) {
        ...
        if(!isset($weekly_data[$yearWeek])) {
            ...
            $weekly_data[$yearWeek] = ['range' => $mon . ' ~ ' . $sun, 'pnl' => 0, 'equity_avg' => 0, 'days' => 0];
        }
    ...
    $html = '<div class="table-responsive"><table class="table table-hover text-center align-middle"><thead class="table-light"><tr><th>' . t('th_week_range') . '</th><th>' . t('th_week_pnl') . '</th><th>' . t('th_avg_amt') . '</th><th>' . t('th_week_roi') . '</th><th>' . t('th_txid') . '</th></tr></thead><tbody>';
    krsort($weekly_data);
    foreach ($weekly_data as $yw => $data) {
        ...
        // TXID 연동 표기
        $txid_display = isset($txid_map[$yw]) ? htmlspecialchars($txid_map[$yw]) : '<span class="text-muted">-</span>';

        $html .= "<tr><td>{$data['range']}</td><td class='{$pnlClass}'>" . ($data['pnl'] > 0 ? '+' : '') . format_truncate($data['pnl'], 4) . "</td><td>" . format_truncate($avg_eq, 2) . "</td><td class='{$pnlClass}'>" . ($roi > 0 ? '+' : '') . format_truncate($roi, 2) . "%</td><td class='text-primary fw-bold'>{$txid_display}</td></tr>";
    }
    return $html . '</tbody></table></div>';
}

// 폼 UI 렌더링을 위한 주간 태그 목록 생성
$weekly_data_info = [];
...
<head>
    ...
    <style>
        ...
        .top-bar .exchange-badge { background: #20c997; color: #fff; padding: 4px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: 600; }
        .btn-sm-custom { padding: 4px 12px; font-size: 0.8rem; border-radius: 6px; }
        .error-box { background: #fff3cd; border: 2px solid #ffc107; border-radius: 12px; padding: 30px; text-align: center; margin-top: 20px; }
...
<div class="container">
    <div class="top-bar">
        <h2 class="fw-bold mb-0">📊 <?php echo h($exchange_name); ?> <?= t('dash_title') ?></h2>
    ...
    if (isset($_GET['msg'])) {
        ...
        if ($msg_val === 'update_ok') {
            ...
            $alert_type = "danger";
        } elseif (strpos($msg_val, 'withdraw_ok_') === 0) {
            $wid = htmlspecialchars(urldecode(substr($msg_val, 12)));
            $alert_msg = t('msg_withdraw_ok') . " (ID: {$wid})";
        } elseif (strpos($msg_val, 'withdraw_fail_') === 0) {
            $fail_reason = htmlspecialchars(urldecode(substr($msg_val, 14)));
            $alert_msg = t('msg_withdraw_fail') . " {$fail_reason}";
            $alert_type = "danger";
    ...
    <?php if ($fatal_error): ?>
        <div class="error-box">
            ...
            <div class="mt-3">
                <a href="<?php echo htmlspecialchars($current_page); ?>?s_symbol=<?php echo h($s_symbol); ?>" class="btn btn-warning">🔄 <?= t('btn_retry') ?></a>
                <a href="logout.php" class="btn btn-outline-secondary ms-2"><?= t('dash_logout') ?></a>
            </div>
        </div>
    <?php endif; ?>
        ...
        <div class="tab-content" id="dashboardTabsContent">
            ...
            <div class="tab-pane fade show active" id="tab-dash" role="tabpanel">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <div class="card p-3 card-spot">
                            ...
                            <div class="balance-value mt-2"><?php echo format_truncate(($wallet_spot['free'] ?? 0) + ($wallet_spot['locked'] ?? 0), 4); ?></div>
                            <div class="mt-1 opacity-75"><?= t('lbl_use_avail') ?> <?php echo format_truncate($wallet_spot['free'] ?? 0, 4); ?></div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-hover mb-0 text-center align-middle" id="current-position-table">
                            ...
                            <tbody>
                                <?php foreach ($current_list as $row):
                                    ...
                                    $initial_qty = rx_get_bot_initial_qty_websea($keys, $row['symbol'] ?? '');
                                    $martingale_count = ($initial_qty > 0 && $real_qty > 0) ? round($real_qty / $initial_qty) : 0;
                                    $martingale_text = $martingale_count > 0 ? "<span class=\"badge bg-success\">{$martingale_count}" . t('lbl_times') . "</span>" : "-";

                                    $netUnrealizedPnL = floatval($row['unrealizedPnL'] ?? 0);
                                    $netRoi = floatval($row['profitRate'] ?? 0);
                                ...
                                <tr>
                                    ...
                                    <td>x<?php echo htmlspecialchars($row['leverage']); ?></td>
                                    <td><?php echo format_truncate($initial_qty, 3); ?></td>
                                    <td><?php echo format_truncate($real_qty, 3); ?></td>
                                    <td><?php echo $martingale_text; ?></td>
                                    <td><?php echo format_truncate($row['avgPrice'] ?? 0, 1); ?></td>
                                    <td><?php echo format_truncate($row['markPrice'] ?? 0, 1); ?></td>
                ...
                <?php if (!empty($history_list_page)): ?>
                    ...
                    <div class="card table-responsive p-0">
                        <table class="table table-hover mb-0 text-center align-middle">
                            <thead class="table-dark">
                                <tr>
                                    <th><?= t('th_close_time') ?></th><th><?= t('th_symbol') ?></th><th><?= t('th_position') ?></th><th><?= t('th_leverage') ?></th>
                                    <th><?= t('th_entry_px') ?></th><th><?= t('th_close_px') ?></th><th><?= t('th_real_pnl') ?></th><th><?= t('th_roi') ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($history_list_page as $row):
                                    $symbol_short = explode('-', strtoupper($row['symbol'] ?? '-'))[0];
                                    $sideClass = (isset($row['side']) && $row['side'] == 'LONG') ? 'text-long' : 'text-short';

                                    $pnl = floatval($row['realizedPnL'] ?? 0);
                                    $pnlClass = ($pnl >= 0) ? 'text-profit' : 'text-loss';
                                ?>
                                <tr>
                                    <td><?php echo isset($row['closeTime']) ? date('Y-m-d H:i:s', $row['closeTime'] / 1000) : '-'; ?></td>
                                    <td><?php echo h($symbol_short); ?></td>
                                    <td class="<?php echo $sideClass; ?>"><?php echo h($row['side'] ?? '-'); ?></td>
                                    <td>x<?php echo htmlspecialchars($row['leverage'] ?? 50); ?></td>
                    ...
                    <?php
                    $get_pagging_add = $admin_view ? "admin_view={$_GET['admin_view']}&admin_uid={$_GET['admin_uid']}&admin_ce={$current_ce}&" : "";
                    if ($total_pages > 1):
                    ?>
                ...
                <div class="calendar-container" id="rxCalendar">
                    <div class="calendar-header">
                        <div class="calendar-nav">
                            <a href="<?php echo $prevUrl;?>" class="btn btn-outline-primary btn-sm" onclick="showTab('tab-cal')">&laquo; <?= t('btn_prev_mo') ?></a>
                            <h4 class="mb-0 mx-3"><?php echo $cal_year; ?><?= t('cal_year') ?> <?php echo sprintf('%02d', $cal_month); ?><?= t('cal_month') ?></h4>
                            <a href="<?php echo $nextUrl;?>" class="btn btn-outline-primary btn-sm" onclick="showTab('tab-cal')"><?= t('btn_next_mo') ?> &raquo;</a>
                        </div>
            ...
            <?
                $add_params = $admin_view ? "admin_view=1&admin_uid=" . urlencode($admin_uid) . "&admin_ce=" . $current_ce . "&" : "";
            ?>
            <div class="tab-pane fade" id="tab-settings" role="tabpanel">
                <div class="row">
                    <div class="col-lg-4 col-md-6 mb-3">
                        ...
                        <div class="card p-4 h-100">
                            <!-- [수정됨] id 추가 및 기본 알림 제거 -->
                            <form method="POST" action="?s_symbol=<?php echo urlencode($s_symbol); ?>&<?=$add_params?>#tab-settings" id="settingsForm">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group mb-2">
                                        <input type="text" name="up_acount" class="form-control" value="<?php echo number_format(floatval($member['rx_acount'] ?? 0)); ?>" required oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, ',');">
                                        <span class="input-group-text">USDT</span>
                                    </div>
                                    <label class="form-label fw-bold"><?= t('lbl_spot_amt') ?></label>
                                    <div class="input-group">
                                        <input type="text" name="up_spot_acount" class="form-control" value="<?php echo number_format(floatval($member['rx_spot_acount'] ?? 0)); ?>" required oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, ',');">
                                        <span class="input-group-text">USDT</span>
                                    </div>
                                    <small class="text-muted mt-2 d-block"><?= t('lbl_fut_amt_desc') ?></small>
                                </div>
                    ...
                    <div class="col-lg-4 col-md-12 mb-3">
                        <h4 class="section-title">📤 <?= t('title_withdraw') ?></h4>
                        <div class="card p-4 h-100">
                            <form method="POST" action="?s_symbol=<?php echo urlencode($s_symbol); ?>#tab-settings" onsubmit="return confirm('<?= t('confirm_withdraw') ?>');">
                                ...
                                <div class="row">
                                    <div class="col-6 mb-2">
                                        <label class="form-label fw-bold"><?= t('lbl_currency') ?></label>
                                        <input type="text" name="w_currency" class="form-control" value="USDT" required>
                                    </div>
                                    <div class="col-6 mb-2">
                                        <label class="form-label fw-bold"><?= t('lbl_chain') ?></label>
                                        <input type="text" name="w_chain" class="form-control" value="TRC20" required>
                                    </div>
                                </div>
                                <div class="mb-2">
                                    <label class="form-label fw-bold"><?= t('lbl_address') ?></label>
                                    <input type="text" name="w_address" class="form-control" placeholder="<?= t('ph_address') ?>" required>
                                </div>
                                <div class="mb-2">
                                    <label class="form-label fw-bold"><?= t('lbl_withdraw_amt') ?></label>
                                    <input type="number" step="0.01" name="w_amount" class="form-control" placeholder="<?= t('ph_amt') ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-bold"><?= t('lbl_memo') ?></label>
                                    <input type="text" name="w_memo" class="form-control" placeholder="<?= t('ph_memo') ?>">
                                </div>
                                <button type="submit" class="btn btn-warning w-100 fw-bold mt-auto"><?= t('btn_withdraw') ?></button>
                            </form>
                        </div>
                    </div>
                </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title fw-bold" id="balanceCheckModalLabel">💰 자산 설정 확인</h5>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary fw-bold" id="btnConfirmSettings">확인</button>
      </div>
    </div>
  </div>
</div>
...
<script>
// JS 환경에서도 소수점 절사 후 1000단위 콤마 추가 적용
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (tableBody) {
        ...
        function updatePositionTable(positions) {
            ...
            positions.forEach(row => {
                ...
                let martingaleCount = (initialQty > 0 && realQty > 0) ? Math.round(realQty / initialQty) : 0;
                ...
                let netUnrealizedPnL = parseFloat(row.unrealizedPnL || 0);
                let netRoi = parseFloat(row.profitRate || 0);
                let lever = parseInt(row.leverage || 50);
    ...
    if (btnCheckAndSave && settingsForm) {
        btnCheckAndSave.addEventListener('click', function() {
            ...
            // 버튼 상태 임시 변경 (중복 클릭 방지)
            let originalText = btnCheckAndSave.innerHTML;
            btnCheckAndSave.innerHTML = "⏳ Exchange loading...";
            ...
            fetch(fetchUrl)
                ...
                .then(data => {
                    ...
                    if (data.status) {
                        ...
                        // 소수점을 버리고 정수로만 콤마 포맷팅
                        const fmtInt = (num) => Math.floor(num).toLocaleString('en-US');

                        let htmlContent = `
                            <div class="mb-2" style="font-size:1.05rem;">
                                거래소 선물자산 <strong>$ ${fmtInt(actFut)}</strong> &nbsp;|&nbsp; 설정금액 <strong class="text-primary">$ ${fmtInt(inputFut)}</strong>
                            </div>
                            <div class="mb-3" style="font-size:1.05rem;">
                                거래소 현물자산 <strong>$ ${fmtInt(actSpot)}</strong> &nbsp;|&nbsp; 설정금액 <strong class="text-primary">$ ${fmtInt(inputSpot)}</strong>
                            </div>
                        `;

                        // 선물과 현물을 각각 비교하여 경고 문구 세분화
                        let isFutOver = inputFut > actFut;
                        let isSpotOver = inputSpot > actSpot;
                        let warningMsg = "";

                        if (isFutOver && isSpotOver) {
                            warningMsg = "입력하신 설정금액이 선물과 현물 모두 실제 보유자산보다 많습니다.";
                        } else if (isFutOver && !isSpotOver) {
                            warningMsg = "입력하신 설정금액이 선물은 많고, 현물은 작습니다.";
                        } else if (!isFutOver && isSpotOver) {
                            warningMsg = "입력하신 설정금액이 현물은 많고, 선물은 작습니다.";
                        }
                        ...
                        if (warningMsg !== "") {
                            ...
                            htmlContent += `
                                <div class="alert alert-info mb-0 py-3" style="border-radius: 8px;">
                                    ℹ️ 보유 자산이 충분합니다. 입력하신 내용으로 세팅하시겠습니까?
                                </div>`;
                        }
                        ...
                        // API 응답 에러 (키 설정 오류 등)
                        if (confirm("실제 잔고를 확인할 수 없습니다. 입력한 금액으로 저장하시겠습니까?\n사유: " + (data.msg || "오류"))) {
                            settingsForm.submit();
                        }
                    }
                })
